...
while ($row = mysqli_fetch_array($select_sql_result)) {
    $nickname_sql = "select Name from User where Id = '$row[5]'";
    $nickname_sql_result = mysqli_query($con, $nickname_sql);
    $nickname = mysqli_fetch_array($nickname_sql_result);

    $response[] = array('reviewID' => $row[0], 'content' => $row[1], 'score' => $row[2], 'createdAt' => $row[3], 'restaurantID' => $row[4], 'nickname' => $nickname[0], 'writer' => $row[5]);
}
