...
while ($row = mysqli_fetch_array($select_sql_result)) {
    $restaurant_name_sql = "select name from restaurant where restaurantID = '$row[4]'";
    ...
    $response[] = array('restaurantName' => $restaurant_name[0], 'reviewID' => $row[0], 'content' => $row[1], 'score' => $row[2], 'createdAt' => $row[3], 'restaurantID' => $row[4], 'writer' => $row[5]);
}
